...
然后在终端中使用cargo update更新这些依赖到最新版本

```
cargo update
```

然后在lock文件中查看上述crate的最新版本，并且更新依赖中crate的版本信息
...
使用cargo build 编译（安装依赖）

```
cargo build
```

引入外部包，注意与引入内部包的写法不一样，使用关键字extern，然后才使用use
...
新建一个文件作为主文件的模块项，注意Rust语言中的代码总是以模块的的方式进行组织
